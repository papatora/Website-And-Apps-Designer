...
- `tools/` — `slop-lint.mjs`, `contrast-check.mjs`, `export-tokens.mjs`, `new-project.mjs`, `serve.mjs`.
...
- Adding a skin: copy `_template.css`, fill every variable, add it to `registry.js`, add a `<link>` in `index.html`, run `npm run tokens` and `npm run check`.
